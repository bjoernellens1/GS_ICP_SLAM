...
4. **CI/CD Pipeline (GitHub Actions)**
   ...
   - **Fixed**: Repository name converted to lowercase for registry compatibility
...
├── .github/
│   ├── workflows/
│   │   └── docker-build-push.yml      # CI/CD workflow (with lowercase fix)
...
## Key Bug Fix: Lowercase Registry Names

### Problem
The original GitHub Actions workflow used `${{ github.repository }}` directly, which includes uppercase letters (`bjoernellens1/GS_ICP_SLAM`). Docker registries require lowercase repository names, causing build failures:

```
ERROR: invalid reference format: repository name (bjoernellens1/GS_ICP_SLAM) must be lowercase
```

### Solution
Added a step to convert the repository name to lowercase before using it in registry references:
...
Then use `${{ steps.repo_lowercase.outputs.image_name }}` instead of `${{ env.IMAGE_NAME }}` in all registry-related operations.
...
- **Fixed**: All cache references use lowercase repository names
...
```bash
# Pull pre-built image (using lowercase name)
...
All tags use the lowercase repository name: `ghcr.io/bjoernellens1/gs_icp_slam`
...
### 2026-02-06 - Registry Name Fix

- ✅ Fixed Docker registry name to use lowercase
- ✅ Added repository name conversion step in GitHub Actions
- ✅ Updated all cache references to use lowercase name
- ✅ Fixed build failures due to uppercase repository name
